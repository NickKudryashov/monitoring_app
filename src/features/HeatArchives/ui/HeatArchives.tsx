...
import {
    fetchArchData,
    request_archives,
    request_polling_status,
    request_system_date,
} from "../model/services/fetchHeatArchives/fetchHeatArchives";
...
export const HeatArchives = memo((props: HeatArchivesProps) => {
    ...
    const { data: dev, isLoading } = getHeatDeviceData(dev_id);
    ...
    const downloadPDF = async () => {
        ...
        fetch(`${API_URL}heat_reports/create/${dev_id}`, {
            method: "PUT",
            headers: {
                ...
                <div className={cls.reportBlock}>
                    <div className={cls.formData}>
                        <input
                            ...
                            onChange={(e) => setStartDatReport(e.target.value)}
                            type="date"
                            placeholder="Дата начала:"
                        />
                        <input
                    ...
                    <div>
                        <select
                            value={formArchType}
                            onChange={(e) => setFormArchType(e.target.value)}
                        >
                            <option value="0">Часовые архивы</option>
                            <option value="1">Суточные архивы</option>
                            <option value="2">Месячные архивы</option>
                        ...
                        <div className={cls.infoBlock}>
                            {Object?.values(data ?? [])?.map(
                                (sys: HeatArchivesRecord) => (
                                    <div
                                        className={cls.recordInfo}
                                        key={sys.id}
                                    >
