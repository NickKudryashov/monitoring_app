...
<div className={cls.cards}>{categories.map(cat=> cat.parentID===parentID && <CategoryCard onClick={()=>selectCat(cat.id)} name={cat.name} /> )}</div>
<div className={cls.cards}>{objects.map(obj=> obj.category===parentID && <ObjectCard onClick={()=>selectObj(obj.id)}  name={obj.name} />)}</div>
</div>
 );
}