...
function SimpleReport(props:{deviceData:HeatDevice}):ReactElement {
    ...
    const {data,isLoading} = getArchives(String(deviceData?.id));
    ...
    const [allSettingsMode,setAllSettingsMode] = useState(true);
    ...
    const downloadPDF = async () => {
        ...
        const pre_response = await $api.post<{filename:string}>(`heat_reports/create/${deviceData.id}`,temp);
        ...
        <VFlexBox height="85%" className={cls.SimpleReport}>
            <VFlexBox className={cls.content} alignItems="center" gap="7px">
                ...
                <HFlexBox align="space-around" height="40%">
                    <ArchTypeBlock archData={data} currentArchtype={archType} currentSystem={selectedSystem} onChangeArchtype={setArchType}/>
                    { data && <SystemsBlock archData={data} currentArchtype={archType} currentSystem={selectedSystem} 
                        deviceData={deviceData} sdate={startDate} edate={endDate}
                        setEdate={setEndDate} setSdate={setStartDate} onChangeSystem={setSelectedSystem} />}
                </HFlexBox>
                <AppButon
                    className={classNames(cls.pollBtn,{},[])}
                    onClick={()=>downloadPDF()}
                    theme={AppButtonTheme.SHADOW}
                >сформировать архив</AppButon>
            </VFlexBox>
        </VFlexBox>
    );
}
