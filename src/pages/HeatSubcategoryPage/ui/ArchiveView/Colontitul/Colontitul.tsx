...
import { ColontitulResponse, createUpCol, editDownCol, editUpCol } from "pages/HeatSubcategoryPage/api/api";
...
function Colontitul(props:ColontitulProps):ReactElement {
    ...
    const editName = useCallback((props:EditArgs)=>{
    ...
    console.log("colos ",cols);
    return (
        <VFlexBox>
            <VFlexBox height="90%" gap="5px" alignItems="end" className={cls.Colontitul}>
                {cols?.map((el)=>
                    <HFlexBox height="20%" className={cls.row} alignItems="center" align="space-around" key={el.id}>
                        <AppInput type="checkbox" checked={el.enabled} onChange={(e)=>editcol({id:el.id,enabled:e.target.checked})}/>
                ...
                <AppButon className={cls.btn} height="25%" width="40%" onClick={create} theme={AppButtonTheme.SHADOW} >Добавить</AppButon>
            </VFlexBox>
        </VFlexBox>
    );
}

function KeyField({edit,el}:{edit:(props:EditArgs)=>void,el:ColontitulResponse}):ReactElement {
    ...
    return (
        <AppInput placeholder="Ключ:" onChange={(e)=>onChangeName({id:el.id,name:e.target.value})}value={keycol} className={classNames(cls.input,{},[cls.keyInput])}/>
    );
}

function ValueField({edit,el}:{edit:(props:EditArgs)=>void,el:ColontitulResponse}):ReactElement {
    ...
    return (
        <AppInput placeholder="Значение" onChange={(e)=>onChangeName({id:el.id,value:e.target.value})}className={cls.input} value={keycol}/>
    );
}
