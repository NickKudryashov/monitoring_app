...
function ReportSettings(props: {
    ...
    const newPeriodHandler = () => {
        ...
        console.log(sdate);
        createperiod({
            archive_type: selectedArchtype,
            device_id: deviceData.id,
            system_sub_id: selectedSystem,
            end_day: eday,
            start_day: sday,
        });
    };
        ...
        >
            ...
            <HFlexBox height="5%" width="95%" align="space-around">
                <AppButon
                    ...
                    onClick={() => updateParamBySchema({ id: selectedSystem })}
                >
                    Выставить параметры под схему
                </AppButon>
                <AppButon
                    className={cls.btn}
                    width="40%"
                    height="100%"
                    theme={AppButtonTheme.SHADOW}
                    onClick={() => updateColsByDate({ id: selectedSystem })}
                >
                    Загрузить колонтитулы по умолчанию
                </AppButon>
            </HFlexBox>
            <HFlexBox
                align="center"
            ...
            >
                ...
                {deviceData?.systems.map((el) => (
                    <HFlexBox
                        ...
                        key={el.id}
                        width="20%"
                        onClick={() => setSelectedSystem(el.id)}
                    >
                        <AppInput
                            type="radio"
                            checked={el.id === selectedSystem}
                        />
            ...
            >
                ...
                <AppButon onClick={newPeriodHandler} className={cls.btn}>
                    Добавить
                </AppButon>
            </HFlexBox>
            <HFlexBox
            ...
            >
                <VFlexBox width="20%">
                    <HFlexBox
                        gap="5px"
                        alignItems="center"
                        onClick={() => setSelectedArchtype(0)}
                    >
                        <AppInput
                            checked={selectedArchtype === 0}
                            type="radio"
                        />
                        <p>часовые</p>
                    </HFlexBox>
                    <HFlexBox
                        gap="5px"
                        alignItems="center"
                        onClick={() => setSelectedArchtype(1)}
                    >
                        <AppInput
                            checked={selectedArchtype === 1}
                            type="radio"
                        />
                        <p>суточные</p>
                    </HFlexBox>
                    <HFlexBox
                        gap="5px"
                        alignItems="center"
                        onClick={() => setSelectedArchtype(2)}
                    >
                        <AppInput
                            checked={selectedArchtype === 2}
                            type="radio"
                        />
                        <p>месячные</p>
                    </HFlexBox>
                ...
                <HFlexBox
                    ...
                    gap="10px"
                    height="95%"
                    className={cls.periodBox}
                >
                    {!periodsLoading &&
                        periods &&
                        periods[selectedArchtype]?.map((el) => (
                            <HFlexBox
                                height={"15%"}
                                alignItems="center"
                                align="space-around"
