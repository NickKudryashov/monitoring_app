...
const ElectroSubcategoryPage = (props: PropsWithChildren<ElectroSubcategoryPageProps>) => {
    const { className } = props;
    const {id} = useParams<{id:string}>();
    const [selectedTab,setSelectedTab] = useState(0);
    const {data:generalData,refetch:refetchGeneral,isLoading} = getSubcatGeneralInfo(id);
    const {data:elData,isLoading:idIsLoading} = getElectroDevId(id);
    const {data:devData,refetch:refetchDev,isLoading:devIsLoading} = getElectroDeviceData(elData ? String(elData[0]) : undefined);
    const poll = useElectroPoll({id:devData?.id,autoPoll:devData?.connection_info.connection_type!=="GSM",onUpdate:()=>{refetchDev();refetchGeneral();}});
    ...
    const downloadXLSFile = async (dev:TopLevelElectroDevice) => {
        ...
        fetch(`${API_URL}electro_report/${dev.id}`,{method:"PUT",headers:{"Authorization":"Bearer "+localStorage.getItem("access_token")}}).then(
            response => {
    ...
    const content = (
        <DetailView>
            <VFlexBox width="90%">
                <PageHeader poll={poll} report={()=>downloadXLSFile(devData)} generalData={generalData}/>
                ...
                <HFlexBox className={cls.contentBox} gap="5px" align="space-between">
                    <SubcategoryTabs
                        ...
                        content={
                            {
                                0:<GeneralInfoBlock device_num={devData?.device_num} device_type_verbose_name={devData?.device_type_verbose_name} systems={devData ? Object.keys(devData.counters_by_can).length : 0} address={generalData?.adress} name={generalData?.user_object_name} />,
