...
export default class UserService {
    ...
    static async checkDefaultLogin() {
        ...
        const response = await axios.post<DefaultAuthCheckResponse>(`${API_URL}auth-refresh/`, {
            access: access_token,
    ...
    static async getUserInfo() {
        const access_token = localStorage.getItem('access_token') ?? 'err'
        let refresh_token = localStorage.getItem('refresh_token') ?? 'err'
        const response = await $api.post<UserData>(`${API_URL}user/`)
        return response
    }
